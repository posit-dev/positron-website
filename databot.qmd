...
In Positron, choose the [Extensions view](extensions.qmd) from the Activity Bar on the left or use the keyboard shortcut {{< kbd mac=Command-Shift-X win=Ctrl-Shift-X linux=Ctrl-Shift-X >}} and search for “Databot". Click the button to install the extension.
...
To acknowledge the research preview status, find [`databot.researchPreviewAcknowledgment`](positron://settings/databot.researchPreviewAcknowledgment) in Positron's settings. In the text box, type "Acknowledged".
