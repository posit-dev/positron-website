...
Positron enables every provider by default and reads the list from `~/.posit/ai/providers.json`, so to hide one, or to allow only a specific set, edit that file. Run _Open AI Provider Settings (JSON)_ to open it, and see [Only allow specific providers](https://assistant.posit.co/docs/reference/providers-config/#only-allow-specific-providers) in the `providers.json` reference for the keys to use.
