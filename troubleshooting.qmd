...
If your R or Python environment is slow to start or failing to start, run the _Runtime Startup Diagnostics_ command from the [Command Palette](command-palette.qmd). This command collects diagnostic information about interpreter startup, including timing data and potential issues that may be causing problems.
...
If an [AI feature](ai-features.qmd) is not working the way you expect, run the _AI: Create Diagnostic Report_ command from the [Command Palette](command-palette.qmd). The report gathers everything needed to debug an AI problem into a single Markdown file and opens it in a new editor tab.
...
::: {.callout-warning}
## Read the report before you share it
