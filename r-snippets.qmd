...
However, in actual usage, `for` is always part of a larger construct:
...
A snippet for `for` inserts this whole skeleton, with special placeholders that let you use <kbd>Tab</kbd> to move through the positions for `variable`, `vector`, and `# code to repeat`.
...
At this time, nothing here is configurable.

![Completion list with rectangles highlighting two entries for the `for` reserved word, one as a bare completion and another as a snippet](images/snippets-keyword-with-two-items.png){width=700}
...
1. "Global" but for one workspace: contains snippets for multiple languages but scoped to a specific workspace. Create with *New Snippets file for 'MyProject'*.
...
1. Language-specific: contains snippets for one specific language. Create by selecting the language from the drop-down menu.
