...
![A Quarto document in Positron with a Python code cell and its plotnine scatter plot of penguin measurements displayed inline, directly below the cell.](images/quarto-inline-output.png){width=800}
...
When inline output is enabled, code in a Quarto document runs with the document's folder as the working directory. This matches how Quarto sets the working directory when it renders a document, so relative file paths behave the same way when you run code interactively and when you render.
