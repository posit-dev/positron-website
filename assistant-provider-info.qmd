...
This provider connects to any OpenAI-compatible endpoint you configure, so its data handling and terms depend entirely on the provider you choose. Review that provider's own privacy policy and terms of service directly.

### Custom providers

A [named custom provider](assistant-providers.qmd#custom-providers) follows the terms of whichever type you gave it. An Anthropic-type entry follows Anthropic's terms above, an OpenAI-type entry follows OpenAI's, and an OpenAI Compatible-type entry follows the endpoint's own terms, as with the built-in provider above.
