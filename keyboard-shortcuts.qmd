...
| {{< kbd mac=Command-Shift-Enter win=Ctrl-Shift-P linux=Ctrl-Shift-P >}} | Run the current file in the Console (using e.g., `source()` for `.R` or `%run` for `.py`); if the file contains [code cells](code-cells.qmd), this runs the current cell instead |
