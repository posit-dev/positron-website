...
| {{< kbd mac=Command-Shift-Enter win=Ctrl-Shift-P linux=Ctrl-Shift-P >}} | Run the current file or [code cell](code-cells.qmd) in the Console (using e.g., `source()` for `.R` or `%run` for `.py`) |
...
For shortcuts specific to the Positron Notebook Editor, see the [notebook keyboard shortcuts](positron-notebook-editor.qmd#keyboard-shortcuts).
