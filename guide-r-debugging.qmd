...
The Breakpoints pane provides checkboxes to break on errors, warnings, or interrupts. These let the debugger pause automatically when a condition is raised, without setting a line breakpoint in advance.

- **Errors**: Pauses execution when an error occurs. This replaces the `options(error = browser)` or `options(error = recover)` pattern.

- **Warnings**: Pauses execution when a warning is emitted. This replaces the `options(warn = 2, error = browser)` pattern.
...
- **Interrupts**: Pauses execution when you interrupt R instead of cancelling the computation. This is useful to investigate why R is taking a long time or why it is stuck in an infinite loop.
    ...
    Check this box, then interrupt R with {{< kbd Ctrl-C >}} or by selecting the interrupt button in the Console. You can inspect the call stack and variables, then resume the computation as if R had not been paused at all by selecting **Continue** in the debug toolbar or sending `c` in the Console.

When the debugger pauses on an error or warning, the condition type and message appear in the editor and the debug UI. The initial pause location is typically inside internal error handling or emission code, which is not always useful on its own. Use the **Call Stack** view to navigate to the frame where the error or warning originated. Selecting **Continue** resumes execution and propagates the condition normally.
...
Prefix an expression with `/print` to display the full printed R output instead of a structured variable. For example, `/print summary(model)` shows the printed summary. The result is truncated to a single line in the Watch pane, but you can hover over it with the mouse to see the full output.
...
By default, Shiny internal frames are hidden from the Call Stack view so you can focus on your own code. To show Shiny internals, set the global option:

```r
options(ark.debugger.show_hidden_frames = "fenced")
```

Set it to `TRUE` to show all kinds of hidden frames, not just Shiny ones.
