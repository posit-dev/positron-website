...
The dialog displays the following information about the active interpreter session:
...
![Interpreter session status icons from **Console** pane](images/console-pane-interpreter-session-status.png){fig-alt="Three status indicators: green circle for idle, blue spinner for busy, and red circle for shutdown."}