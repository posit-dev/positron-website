...
## Where provider configuration lives {#provider-configuration-file}

Provider configuration lives in a single file, `~/.posit/ai/providers.json`. Positron writes it for you as you connect and edit providers, so most people never need to open it. Your API keys and tokens are not in this file. Positron stores them separately.
...
- **Configuration:** the AWS region and profile, if your environment does not already set them (see below)
...
Set either value with the `AWS_REGION` and `AWS_PROFILE` environment variables before you launch Positron. To set them for Positron only, use the `aws.region` and `aws.profile` keys under `providers.bedrock` in `providers.json`. See [`aws`](https://assistant.posit.co/docs/reference/providers-config/#aws) in the `providers.json` reference.
...
Set the capability fields to match your deployment. For what each field means and the optional ones this example leaves out, see [Model fields](https://assistant.posit.co/docs/reference/providers-config/#model-fields) in the `providers.json` reference.
...
To change those entries, or to set the capability fields for each model, edit that block directly. See [Choosing which models appear](https://assistant.posit.co/docs/reference/providers-config/#choosing-which-models-appear) in the `providers.json` reference.
...
For the project and location keys, see [`googleCloud`](https://assistant.posit.co/docs/reference/providers-config/#googlecloud) in the `providers.json` reference.
