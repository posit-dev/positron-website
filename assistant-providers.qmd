...
A connected entry shows under **Connected Providers** with a **Custom** badge, and in the model picker and {{< fa circle-user >}} **Accounts** menu as `<name> (Custom Provider)`. If it errors, it moves to **Needs Attention**. If you disconnect it, it moves to **Custom Providers** until you reconnect.
...
To remove an entry, delete that block. **Disconnect** only clears the stored credential. The entry stays in the list, ready to reconnect.
