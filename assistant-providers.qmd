...
Set it with the `AWS_REGION` environment variable before you launch Positron, or, to set it for Posit Assistant only, with the `aws.region` key under `providers.bedrock` in `providers.json`. See [`aws`](https://assistant.posit.co/docs/reference/providers-config/#aws) in the `providers.json` reference.
...
Set it with the `AWS_PROFILE` environment variable before you launch Positron, or, to set it for Posit Assistant only, with the `aws.profile` key under `providers.bedrock` in `providers.json`. See [`aws`](https://assistant.posit.co/docs/reference/providers-config/#aws) in the `providers.json` reference.
