...
## Databricks [Experimental]{.provider-badge .experimental} {#databricks}
...
Create a personal access token in your Databricks workspace. In the Databricks provider dialog, enter the **Workspace URL** and **Personal Access Token**. Positron checks the token with the Databricks workspace before saving it.
...
::: {.callout-note}
When several Databricks credential sources are available, Positron uses them in this order: Posit Workbench managed credentials, `DATABRICKS_TOKEN` environment variable, `.databrickscfg` profile, then an interactive prompt.
:::
...
## Custom providers [Experimental]{.provider-badge .experimental} {#custom-providers}
