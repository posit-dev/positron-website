...
Posit Assistant connects to a range of language model providers. To connect one, run the command _Authentication: Configure Language Model Providers_, select the provider, and authenticate. For the full setup flow, see the [Getting Started](assistant-getting-started.qmd) guide.

This page covers what each provider needs: the account to have ready, how you authenticate, and any Positron settings to configure first. For an overview of which providers support chat and code completions, see the [provider table](assistant-getting-started.qmd#configure-a-language-model-provider).

Providers marked [Preview]{.provider-badge .preview} or [Experimental]{.provider-badge .experimental} are still being refined and may change.
...
To set either value, add them to the [`authentication.aws.credentials`](positron://settings/authentication.aws.credentials) setting as items with the keys `AWS_REGION` and `AWS_PROFILE`. These override any matching environment variables already set on your system for Positron only; they do not affect your terminal or other tools. Changes require a restart to take effect.
