...
::: {.callout-note}
The **Add Custom Provider** button appears once your installed Posit Assistant supports named custom providers. If you do not see it, update Posit Assistant.
:::

### Add a custom provider

1. Run _Authentication: Configure Language Model Providers_ and select **Add Custom Provider**.
1. Enter a **Provider Name**. It appears in the model picker, and you cannot rename it later. To rename an entry, delete it and add it again.
1. Pick a **Provider Type**: **OpenAI Compatible**, **Anthropic**, or **OpenAI**. The type decides the fields you fill in next, since a custom entry reuses that provider's own connection fields (for example, an Anthropic-type entry asks for the same API key as the built-in Anthropic provider).
1. Enter the credential and base URL, if the type asks for them, and connect.

**OpenAI Compatible**, **Anthropic**, and **OpenAI** are the only types Positron offers here. An entry of a different type, such as `aws` or `ollama`, does not register in Positron, even if you add it to `providers.json` directly.
...
To remove an entry for good, use **Delete** from its row or its connected view. This removes the entry and its stored credential together, and cannot be undone. **Disconnect** is different: it clears the credential but keeps the entry, so reconnecting only asks for the credential again.
...
Each entry is a block under `providers.custom`, keyed by the name you chose:
...
## OpenAI Compatible [Experimental]{.provider-badge .experimental} {#custom-provider}

This provider works with any [OpenAI-compatible](https://ai-sdk.dev/providers/openai-compatible-providers) API endpoint that uses the `/v1/chat/completions` endpoint for chat. We do not recommend using a local model here. Read more about why [local models are not there (yet)](https://posit.co/blog/local-models-are-not-there-yet/) on the Posit blog.
