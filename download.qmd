...
checksums <- list(
  full = c(
    checksum_data[[glue("Positron-{params$version}-Setup.exe")]],          # Windows system level
    checksum_data[[glue("Positron-{params$version}-UserSetup.exe")]],      # Windows user level
    checksum_data[[glue("Positron-{params$version}.dmg")]],               # MacOS universal
    checksum_data[[glue("Positron-{params$version}-x64.deb")]],          # Debian x64
    checksum_data[[glue("Positron-{params$version}-arm64.deb")]],        # Debian arm64
    checksum_data[[glue("Positron-{params$version}-x64.rpm")]],          # Red Hat x64
    checksum_data[[glue("Positron-{params$version}-arm64.rpm")]]         # Red Hat arm64
  ),
  short = substr(c(
    checksum_data[[glue("Positron-{params$version}-Setup.exe")]],          # Windows system level
    checksum_data[[glue("Positron-{params$version}-UserSetup.exe")]],      # Windows user level
