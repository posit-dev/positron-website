...
This guide pertains to the R versions that appear in the top-right interpreter chooser (also known as the _Top Bar Interpreter Dropdown_ internally).
...
One way to navigate to these settings is to use the command palette to do *Preferences: Open Settings*.
Then go to *Extensions > R > Positron R Language Pack (advanced)*.
It's important to use concrete paths in these settings and not rely on shell features, such as tilde (`~/`) or parameter (`${HOME}`) expansion.
...
We highly recommend rig for managing R installation, especially for macOS users who would like to have more than 1 functioning R installation.
