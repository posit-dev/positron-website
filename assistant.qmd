...
Two options are available on errors in the Console: Fix and Explain. Clicking one of these options sends your session to Assistant, which then immediately starts on a solution; details on each of these are available below. 

Assistant automatically receives the appropriate context, including the traceback you're troubleshooting, your recent code inputs, and the active file. You can respond to Assistant after it provides feedback, allowing you to tailor code in Fix mode or ask addition queries in Explain mode. 

All of these interactions are triggered directly from within the Console and exchanges with Assistant happen within the Quick Chat interface. This is separate from the standard chat in the sidebar, allowing you to multitask without interrupting either your Console or Assistant flows. You can pull open the Quick Chat at any time to explore your Fix and Explain history by search for `Open Quick Chat` in the command palette. 

When using Fix and Explain, since Assistant starts immediately, the Quick Chat inherits the following settings from the primary sidebar chat interface:  

- Selected model
- Files added or removed from context 
- Whether the Console session is visible or hidden
...
Console actions are enabled by default when both Assistant is enabled and you are signed into a model provider. If you want to disable these actions while still using other Assistant features, toggle the setting [`positron.assistant.consoleActions.enable`](positron://settings/positron.assistant.consoleActions.enable). This will remove the actions from any errors which appear in the Console, and can be turned on or off at any time.
