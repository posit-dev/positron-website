...
After creating your API key, use it to [add Anthropic as a language model provider](#adding-language-model-providers) in Positron.
...
For more details on chat modes and other chat features, see [Get started with chat in VS Code](https://code.visualstudio.com/docs/copilot/chat/copilot-chat#_switch-between-agents).
...
Once you have created a custom chat mode, it will appear in the mode selector dropdown in the Chat pane, allowing you to select and use it like the default [chat modes](#chat-modes).
...
Positron Assistant looks for these files and appends each file's content to the default prompt for all chat and inline chat interactions. This customization does not affect code completions. Note that the order in which these files are appended is not guaranteed. Follow issue [#10571](https://github.com/posit-dev/positron/issues/10571) for updates.
