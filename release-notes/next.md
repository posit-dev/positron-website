...
We redesigned Positron's welcome page. It now leads with an environment setup card that checks whether Python and R are ready to use, with actions to help resolve any problem it finds. A new "Get Started with Positron" walkthrough covers the Positron panes, keyboard shortcuts, built-in extensions, and Git, and the page links to the walkthroughs from a banner.

#### Import Data

Importing data via the Positron UI is currently our most upvoted feature request, and this release delivers a first version. When you view a CSV or TSV file in the Data Explorer, a new **Import Data** button in the action bar opens a dialog. The dialog shows the code to load that file into a data frame. You can copy the code, or click **Import** to run it in the console, starting a Python session if one is not already running. This first version supports loading CSV and TSV files into Python sessions with the pandas package, with R support and more entry points on the way.
...
Console code submission is now faster, always shows visual feedback, and can be cancelled while a completeness check is in flight. The Console breaks multi-statement input into complete expressions and executes them statement by statement for languages that support it. A new `console.promptWhenIncomplete` setting runs submitted code immediately without a completeness check.
...
- [[#8915](https://github.com/posit-dev/positron/issues/8915)] Console: code submission is now faster, always shows visual feedback, and can be cancelled while a completeness check is in flight.
...
- [[#12850](https://github.com/posit-dev/positron/issues/12850)] Workbench: Positron now loads the environment modules used by the active R or Python interpreter in integrated terminals, so tools launched there (Shiny, Quarto preview, `Rscript`, `python`, and so on) use the same interpreter as the console.
- [[#14950](https://github.com/posit-dev/positron/issues/14950)] Data Explorer: added an **Import Data** button to the Data Explorer.
...
- [[#14844](https://github.com/posit-dev/positron/issues/14844)] Quarto: fixed the **Fix** and **Explain** buttons drawing over the editor when re-running a cell that errors.
...
- [[#15601](https://github.com/posit-dev/positron/issues/15601)] App Launcher: Shiny for R apps that take longer to start than the URL detection timeout are no longer stopped before they finish starting, and Positron previews them when their URL finally appears.
