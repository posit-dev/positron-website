...
<script>
(function () {
  var STEP = 60; // ms between progress updates
  ...
  function init(root) {
    ...
    var index = 0, elapsed = 0, paused = false, stopped = reduced, timer = null;

    function progressPct() {
      return stopped ? 100 : Math.min(100, (elapsed / duration) * 100);
    }
    ...
    function paint() {
      ...
      segs.forEach(function (b, i) {
        ...
        b.tabIndex = on ? 0 : -1;
        var fill = b.querySelector('.pt-carousel__segFill');
        if (fill) {
          fill.style.width = i < index ? '100%' : (on ? progressPct() + '%' : '0%');
        }
      });
    }

    function tickFill() {
      var fill = segs[index] && segs[index].querySelector('.pt-carousel__segFill');
