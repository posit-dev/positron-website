...
Databot is deprecated. The AI chat and coding experience in Positron is Posit Assistant.
...
For additional context, see the [Databot introduction blog post](https://posit.co/blog/introducing-databot/) and [Databot is not a flotation device blog post](https://posit.co/blog/databot-is-not-a-flotation-device/).

[![Welcome page of Databot and suggested first actions to take, like checking loaded data or loading new data](images/databot-welcome.png)](images/databot-welcome.png "Databot Welcome Page")

Databot Welcome Page
...
First, make sure you have a recent version of Positron (2025.08.0 is the minimum version) and have enabled [Positron Assistant](assistant.llms.md).

### Install the Databot extension

In Positron, choose the [Extensions view](extensions.llms.md) from the Activity Bar on the left or use the keyboard shortcut and search for “Databot”. Select the button to install the extension.

### Enable Databot

Databot is an experimental product in research preview and is not ready for production use. To enable Databot, you must first acknowledge this.

To acknowledge the research preview status, find [`databot.researchPreviewAcknowledgment`](positron://settings/databot.researchPreviewAcknowledgment) in Positron’s settings. In the text box, enter “Acknowledged”.

OR

- In Positron, open the Command Palette and run *Open User Settings*.
- In the search bar, enter “Databot”.
- You will see **Databot: Research Preview Acknowledgment**. In the text box, enter “Acknowledged”.

### Start Databot

In Positron, open the Command Palette and run *Open Databot*.

### Enable Positron Assistant

If [Positron Assistant](assistant.llms.md) is not yet enabled, you will see the following message in the Databot panel: “To use Databot, enable Positron Assistant and Anthropic provider.”

Select the button labeled **Help me set it up**. This will open a dialog box.

In section 2 of the dialog box, **Enable Positron Assistant**, if it says “Positron Assistant is not yet enabled” and there are any red checkboxes, then select the button labeled **Update settings**. The checkboxes will turn green.
...
To start Databot in another Positron session, open the Command Palette and run *Open Databot*.
...
- Databot is intended for experienced R and Python users who are familiar with data analysis.
...
- [Positron Assistant](assistant.llms.md) is an *AI coding assistant*; it helps you write code or modify code on disk and can execute arbitrary code in Agent mode.
