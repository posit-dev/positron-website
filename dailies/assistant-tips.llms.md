...
Learn best practices for using Positron Assistant effectively. Explore tips for better prompts, managing conversations, and understanding key AI assistant terms.
...
You might want to start a new conversation when:

- Beginning a new topic or task
- Your chat conversation has become lengthy and the context is less relevant
- You want to reset the context of Assistant to avoid confusion from previous messages
...
2.  Select the **** button in the top right of the chat pane or use the keyboard shortcut in the chat pane.

### Viewing previous chat conversations

To view your chat conversations:

1.  Select the clock icon in the top right of the chat pane, or run the *Chat: Show Chats…* command in the Command Palette.
...
2.  Search for the setting [`positron.assistant.showTokenUsage.enable`](positron://settings/positron.assistant.showTokenUsage.enable).
3.  Check the box to enable token usage display.
    - Note that not all language model providers support token usage.
...
1.  Open the Command Palette and run the command *Preferences: Open User Settings* to open the settings.
2.  Search for the setting [`positron.assistant.toolDetails.enable`](positron://settings/positron.assistant.toolDetails.enable).
...
Bring Your Own (API) Key. This means you need to obtain and provide your own API key from the language model provider to use their models with Positron Assistant.
...
A specific version or configuration of a language model provided by a language model provider, such as Anthropic Claude or GitHub Copilot. Different models can have different capabilities, performance characteristics, and pricing. Examples include “Claude Sonnet 4.5” or “GPT-5”.
