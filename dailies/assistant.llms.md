...
| [MCP Servers](https://assistant.posit.co/docs/reference/mcp-servers/) | Connect external tool servers via the Model Context Protocol to extend the assistant with custom tools, such as database access, API integrations, and internal services. Supports local (stdio) and remote (HTTP/SSE) servers with bearer token or OAuth authentication, including servers hosted on Posit Connect. |
