...
::: {.callout-note}
Is this the right tutorial to start with? It picks up where [First data analysis with R in a Quarto document](tutorial-get-started-quarto.qmd) and [Migrate to Positron from RStudio](tutorial-migrate-from-rstudio.qmd) leave off. If you have not worked through those, start there. This tutorial assumes you already know how to:
...
![The New Folder from Git helper in Positron](images/tutorial-r-ai-git.png){fig-alt="The Positron IDE displaying the New Folder from Git wizard."}
...
::: {.callout-tip title="Try it now"}
1. Open the Console (**View > Console**). If Positron has not yet started an R session, run _Interpreter: Start New Console Session_, and select an R interpreter.
2. Run `renv::activate()` in the Console to activate renv.
3. Run _Interpreter: Restart Active Interpreter Session_ to restart the console.
...
![The Restart R button in the Console pane, for after `renv::activate()`](images/tutorial-r-ai-renv.png){width=500 fig-alt="The Positron R Console showing the Restart R button."}
...
![Posit Assistant open in the left sidebar](images/tutorial-r-ai-assistant.png){fig-alt="Positron with Posit Assistant loaded in the left sidebar."}
...
::: {.callout-note}
Later in this tutorial you will also use code completions, which require [Posit AI Pass](assistant-providers.qmd#posit-ai) or [GitHub Copilot](assistant-providers.qmd#github-copilot). Connecting one of those providers now lets you try that step when you reach it.
:::
...
![The quarterly sales chart you will ask Posit Assistant to build](images/tutorial-r-ai-chart.png){width=500 fig-alt="A bar chart showing quarterly sales over time, with a superimposed line chart."}
...
![Posit Assistant asking permission to run code](images/tutorial-r-ai-permission.png){width=400 fig-alt="A chat with Posit Assistant asking permission to run lines of code."}
...
![The table and chart Posit Assistant generated](images/tutorial-r-ai-results.png){fig-alt="The Positron IDE showing a chart in the chat and a table in the viewer sidebar."}
...
![The exported Quarto document open in the editor beside its rendered preview](images/tutorial-r-ai-report.png){fig-alt="Positron showing a Quarto document in the editor with a rendered HTML preview in the Viewer pane."}
...
![Posit Assistant explaining a cell error in a Quarto document](images/tutorial-r-ai-explain-error.png){width=600 fig-alt="A Posit Assistant chat that explains the cause of the error seen in the Quarto document beside it."}
...
::: {.callout-tip title="Try it now"}
Add a new R code cell at the end of the document with {{< kbd mac=Command-Shift-I win=Ctrl-Shift-I linux=Ctrl-Shift-I >}}. Start typing a line that reports the quarter with the highest sales, such as:
...
2. Press {{< kbd Tab >}} to accept it, or keep typing to dismiss it.
3. Run the cell to confirm it returns the top quarter.

**You will know it worked when** a greyed-out suggestion appears as you type and becomes real code once you press {{< kbd Tab >}}.

:::

![A code completion shown as ghost text in a Quarto cell](images/tutorial-r-ai-completion.png){width=500 fig-alt="A Quarto code cell showing a greyed-out inline code completion suggestion."}
...
![A donut chart added by Posit Assistant](images/tutorial-r-ai-donut.png){fig-alt="Positron showing a preview of the quarto document, which now contains a section with a donut chart."}
...
![The finished dashboard](images/tutorial-r-ai-dashboard.png){fig-alt="A two-column Quarto dashboard showing a quarterly sales table and a bar-and-line chart of sales over time, with a light and dark mode toggle."}
