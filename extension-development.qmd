...
To run extension-host integration tests against the live `positron.*` API—without building Positron from source—use the [`@posit-dev/positron-test-electron`](https://github.com/posit-dev/positron-test-electron) package. It downloads and launches a real Positron build (daily or stable) and runs your test suite against it, the same way [`@vscode/test-electron`](https://code.visualstudio.com/api/working-with-extensions/testing-extension) does for VS Code.
...
Use `channel: 'daily'` to test against upcoming changes, or `channel: 'stable'` (or pin a specific `version`, such as `2026.08.0-365`) to test against released builds. This makes it straightforward to run your Positron API tests in CI across macOS, Windows, and Linux.
