...
There are a few ways to open the Data Explorer. If you want to look at data you have loaded into memory already, you can navigate to the [Variables Pane](variables-pane.qmd) and click on the {{< fa table >}} icon for a specific dataframe object.
...
Directly opening `.csv`, `.tsv`, and `.parquet` files (using DuckDB) is supported by clicking on a file in the File Explorer or using the Command Palette.^[Use the **File Options** button in the action bar to configure how a `.csv` or `.tsv` file is parsed by DuckDB.] GZIP-compressed CSV files ending in `.gz` can also be opened. We may add support for more file types in the future.
