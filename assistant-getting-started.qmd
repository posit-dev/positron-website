...
Foundry does not support automatic model discovery. Positron Assistant defaults to the `model-router` identifier, but you can configure specific models. See [Configure custom model overrides](#configure-custom-model-overrides-msfoundry) below.
:::
...
Google Vertex AI provider support is experimental. Opt in to the [`assistant.provider.googleVertex.enable`](positron://settings/assistant.provider.googleVertex.enable) setting to enable it.
