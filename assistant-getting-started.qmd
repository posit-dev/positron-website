...
**[Posit Assistant](https://pos.it/assistant)** is the AI chat experience in Positron. Available as a preview feature in Positron 2026.04.0 and later, it's the recommended way to use AI in Positron. To get set up, see the [Posit Assistant documentation](https://pos.it/assistant-docs).

::: {.callout-warning}
## Positron Assistant is deprecated

**Positron Assistant**, the original AI extension in Positron, has been superseded by Posit Assistant and will be removed in a future release. We recommend using Posit Assistant instead. Positron Assistant is still available for now, and the rest of this guide covers it for anyone who still needs it.
:::
...
| Snowflake Cortex | {{< fa check >}} | | API Key or Posit Workbench Managed Credentials |
| Microsoft Foundry (Preview) | {{< fa check >}} | | API Key or Posit Workbench Managed Credentials |
| OpenAI | {{< fa check >}} | | API Key |
| Google Vertex AI (Experimental) | {{< fa check >}} | | Google Cloud CLI or Service Account Credentials |
...
1. Select **Sign in** so Positron can verify your AWS CLI authentication.
...
### Configure custom model overrides {#configure-custom-model-overrides-msfoundry}
...
<details>
<summary>Sign in with Application Default Credentials (ADC)</summary>
...
<details>
<summary>Sign in with a service account</summary>
...
1. Run the command _Authentication: Configure Language Model Providers_
1. Select **Google Vertex AI** as the model provider.
1. Select **Sign in** to authenticate with Google Vertex AI.

## Custom Provider

::: {.callout-important}

Positron's custom provider support is intended for use with any [OpenAI-compatible](https://ai-sdk.dev/providers/openai-compatible-providers) API endpoint that uses the `/v1/chat/completions` endpoint for chat.
...
Posit Assistant is pre-installed in recent versions of Positron. Once you've configured a provider above, open the chat panel to start a conversation.

::: {.callout-note}
To use **GitHub Copilot** with Posit Assistant, you must also enable Positron Assistant by turning on the [`positron.assistant.enable`](positron://settings/positron.assistant.enable) setting. Posit Assistant relies on Positron Assistant for GitHub Copilot authentication.
:::

For the full setup guide and feature documentation, see the [Posit Assistant documentation](https://pos.it/assistant-docs).

### Positron Assistant (deprecated)

Positron Assistant is deprecated, and we recommend Posit Assistant instead. If you still need Positron Assistant:
