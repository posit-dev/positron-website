...
1. Have an Azure subscription with a [Microsoft Foundry](https://ai.azure.com/) resource
2. Have one or more models deployed in your Foundry resource
3. Have the endpoint URL and API key for your Foundry resource
...
Foundry does not support the `/models` endpoint, so Positron Assistant defaults to `model-router` as the model identifier. If your resource does not have a Model Router deployment, or if you want to target specific models, configure the [`positron.assistant.models.overrides.msFoundry`](positron://settings/positron.assistant.models.overrides.msFoundry) setting with your deployed model names:
